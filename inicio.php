<div class="menu_top">
	<div class="row row_right">
		<div class="col-sm-8 col-md-offset-2">
			<center>
				<a href="index.php"><img src="img/logo.png" class="logo"></a>
			</center>
		</div>
		<div class="col-sm-2 menu">
			<a href=""><b>Home</b></a>
			<a href="?op=login" class="text_login">Login</a>
		</div>
	</div>
</div>

<div class="imgtext">
	<img src="img/slider.png" class="img1">
	<article>
		<b>APPLAUSO PARA TODOS</b>
		<h1>Soluciones únicas para<br> experiencias inolvidables.</h1>
		<a href="#contacto" class="btn btn-warning btn_cont">Contáctenos</a>
	</article>
</div>

<div class="row menu_down row_right">
	<div class="col-sm-3 col-md-offset-2 col-xs-12">
		<a href="#comohacemos">¿Cómo hacemos?</a>
	</div>
	<div class="col-sm-6 col-xs-12">
		<a href="#comofunciona">¿Cómo funciona?</a>
	</div>
</div>

<div class="row barra_icon row_right">
	<div class="">
		<div class="col-sm-3 col-xs-12">
			<center>
				<img src="img/icon1.png" width="50%">
				<h3>Vive nuevas experiencias</h3>
			</center>
			<p>En Applauso te brinda soluciones de ticketería para eventos, fiestas, teatros, deportes y más.</p>
		</div>

		<div class="col-sm-3 col-xs-12">
			<center>
				<img src="img/icon2.png" width="50%">
				<h3>Aumenta tus ventas</h3>
			</center>
			<p>Applauso te ayuda a aumentar tus ventas, clientela y a crecer con nuestro sitema de ventas Take out.</p>
		</div>

		<div class="col-sm-3 col-xs-12">
			<center>
				<img src="img/icon3.png" width="50%">
				<h3>Trabajemos juntos</h3>
			</center>
			<p>Applauso te da soluciones con nuevas formas de experiencias.</p>
		</div>

		<div class="col-sm-3 col-xs-12">
			<center>
				<img src="img/icon4.png" width="50%" style="padding-bottom: 1em;">
				<h3>Alcanza tus metas</h3>
			</center>
			<p>Applauso te ayuda a consolidar tus metas, con nuevas experiencias.</p>
		</div>
	</div>
</div>

<div id="comohacemos">
	<iframe src="https://drive.google.com/file/d/0B76SMzRmSCO4TS1oOVVHZGxJMG8/preview?info=0&showtitle=0" style="width:100%;max-width:100%;height:480px;margin:0px;padding:0px;border:0px;"></iframe>
	<!--<video id="sampleMovie" src="video/applauso audio 1080.mov" controls width="100%" class="video"></video>-->
</div>	

<div class="container" id="comofunciona">
	<h2 class="text-center titulo">¿CÓMO FUNCIONA?</h2>
	<br><br><br>
	<center><img src="img/smart.png" width="100%"></center>
</div>
<br>

<hr>

<div class="container" style="margin-bottom: 1em;">
<!--	<h2 class="text-center titulo">Tenemos a los mejores de nuestro lado</h2>

	<div class="row">
		<div class="col-sm-4 col-xs-12">
			<center><img src="image/icon5.png" width="50%" style="padding-top: 1em;"></center>
		</div>
		<div class="col-sm-4 col-xs-12">
			<center><img src="image/icon6.png" width="50%"></center>
		</div>
		<div class="col-sm-4 col-xs-12">
			<center><img src="image/icon7.png" width="90%"></center>
		</div>
	</div>-->	
</div>

<br>

<div class="container" id="contacto">
	<div class="row barra_contact">
		<h3 class="text-center">Contáctenos</h3>
	</div>

	<div class="row contact">
		<div class="row">
			<div class="col-sm-8 col-xs-12 col-sm-offset-2">
				<form action="" method="">
					<div class="col-sm-12 col-xs-12">
						<label>Datos de contacto</label>
					</div>
					<div class="col-sm-6 col-xs-12">
						<input type="text" class="form-control" placeholder="Nombre">
					</div>
					<div class="col-sm-6 col-xs-12">
						<input type="text" class="form-control" placeholder="Apellido"><br>
					</div>
					<div class="col-sm-6 col-xs-12">
						<input type="email" class="form-control" placeholder="Correo">
					</div>
					<div class="col-sm-6 col-xs-12">
						<input type="number" class="form-control" placeholder="Número de teléfono"><br>
					</div>

					<div class="col-sm-12 col-xs-12">
						<label class="label_cont">Compañía</label>
					</div>
					<div class="col-sm-6 col-xs-12">
						<input type="text" class="form-control" placeholder="Nombre">
					</div>
					<div class="col-sm-6 col-xs-12">
						<select class="form-control" name="">
							<option>Lugar</option>
						</select>
					</div>

					<div class="col-sm-12 col-lg-9 col-lg-offset-2 col-xs-12">
						<p class="text-center textarealabel">Díganos si actualmente ofrece la entrega y por qué le gustaría asociarse con <br> Applauso.</p>
					</div>
					<div class="col-sm-12 col-xs-12">
						<textarea class="form-control" name="" style="min-height: 150px;"></textarea><br>
					</div>

					<div class="col-sm-12 col-xs-12">
						<center><br><button name="" class="btn btn-warning col-xs-12 col-sm-3 col-sm-offset-5"> Enviar</button></center>
					</div>
				</form>
			</div>
		</div>
	</div>
</div>